...
export class Store {
  constructor(itemName) {
    ...
    this.getStorage = () => JSON.parse(localStorage.getItem(itemName)) || [];
  ...
  find(query, callback) {
    const todoList = this.getStorage();
    let attribute;
    const foundSomething = todoList.filter((item) => {
  ...
  delete(query, callback) {
    const todoList = this.getStorage();
    let attribute;
    const restList = todoList.filter((item) => {
      for (attribute in query) {
        if (query[attribute] !== item[attribute]) {
          return true;
          // 保留跟删除条件不一样的item
...
export function listTemplate(list) {
  return list.reduce((accumalte, item) => `${accumalte}
            <li data-id="${item.id}" >
                <div class="todo-item">
