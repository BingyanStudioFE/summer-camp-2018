...
export default class View {
  ...
  updateItem(targetLabel) {
    const listItem = targetLabel.parentElement.parentElement;
  ...
  toggleCompleteItem(id, completed) {
    const targetItem = util.$qs(`[data-id="${id}"]`);
    if (!targetItem) {
      return;
    }
  ...
  bindDeleteItem(callback) {
    util.$listOn(this.$todoList, "click", ({
      target,
    }) => {
  ...
  bindUpdateItem() {
    util.$listOn(this.$todoList, "dblclick", ({
      target,
    }) => {
  ...
  bindSaveUpdateItem(callback) {
    util.$listOn(this.$todoList, "blur", ({
      target,
    }) => {
      callback(this._getItemId(target), target.value.trim());
    }, "li .update", true);

    util.$listOn(this.$todoList, "keypress", ({
      target,
      keyCode,
    }) => {
      if (keyCode === 13) {
